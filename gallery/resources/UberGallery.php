...
class UberGallery {
    ...
    public function readImageDirectory($directory) {
        ...
        if (!$galleryArray) {
            ...
            $galleryArray['paginator'] = $this->_getPaginatorArray($galleryArray['stats']['current_page'], $galleryArray['stats']['total_pages'], $this->_home);
     ...
     * @access private
     */
    private function _getPaginatorArray($currentPage, $totalPages,$home) {
	$path ="";
		if(isset($home)) {$path=$home.'&amp;';}
        // Set some variables
        ...
        if ($firstPage > 1) {
            $paginatorArray[] = array(
                'text'  => '...',
                'class' => 'more',
                'href'  => $path.'page=' . ($currentPage - $range - 1)
            );
        }
