...
<?php } if ($this->_rootref['S_NO_READ_ACCESS']) {  ?>
	...
	<?php } ?>
		...
		<li class="row<?php if (!($_topicrow_val['S_ROW_COUNT'] & 1)  ) {  ?> bg1<?php } else { ?> bg2<?php } if ($_topicrow_val['S_POST_ANNOUNCE']) {  ?> row2<?php } if ($_topicrow_val['S_POST_STICKY']) {  ?> row2<?php } if ($_topicrow_val['S_TOPIC_REPORTED']) {  ?> reported<?php } ?>">
